...
async def pokemon_info(ctx, *, name: str):
    ...
    if data:
        ...
        # Process evolution details based on the form
        if name.lower() in special_forms:
            evolution_details = process_evolution_chain(evolution_data, special_form=True)
        else:
...
def process_evolution_chain(data, special_form=False):
